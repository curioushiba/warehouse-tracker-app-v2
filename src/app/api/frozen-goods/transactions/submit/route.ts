...
export async function POST(request: Request) {
  try {
    ...
    // Call the submit_fg_transaction database function
    const rpcArgs = {
    ...
    const { data, error } = await supabase.rpc('submit_fg_transaction', rpcArgs as never)
    ...
    // Revalidate frozen goods admin pages
    revalidatePath('/admin/frozengoods')

    return NextResponse.json({ success: true, data })
  } catch (error) {
    console.error('FG Transaction submission error:', error)
    return NextResponse.json(
