...
export async function POST(request: Request) {
  try {
    ...
    // Call the submit_cm_transaction database function
    const rpcArgs = {
    ...
    const { data, error } = await supabase.rpc('submit_cm_transaction', rpcArgs as never)
    ...
    // Revalidate commissary admin pages
    revalidatePath('/admin/commissary')

    return NextResponse.json({ success: true, data })
  } catch (error) {
    console.error('CM Transaction submission error:', error)
    return NextResponse.json(
