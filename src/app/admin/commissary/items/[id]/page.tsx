...
import { getCmItemById } from "@/lib/actions/commissary-items";
...
export default async function CmItemDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id: itemId } = await params;
  const itemResult = await getCmItemById(itemId);

  if (!itemResult.success) {
    return (
      <div className="space-y-6">
        <Link href="/admin/commissary/items">
          <Button variant="ghost" leftIcon={<ArrowLeft className="w-4 h-4" />}>Back to Commissary</Button>
        </Link>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href={`/admin/commissary/items/${item.id}/edit`}>
        ...
        <Card variant="elevated">
          <CardBody className="p-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-[#E07A2F]/10 rounded-lg flex items-center justify-center"><FolderOpen className="w-5 h-5 text-[#E07A2F]" /></div>
              <div>
