...
import { Plus, ChefHat, Search, Package, Archive } from "lucide-react";
import {
  Card,
  CardBody,
  Button,
...
import { getCmItemsPaginated } from "@/lib/actions/commissary-items";
import type { Item } from "@/lib/supabase/types";
import { getStockLevel } from "@/lib/utils";

export default function CommissaryItemsPage() {
  ...
  const fetchItems = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await getCmItemsPaginated({
        page,
        pageSize: 25,
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#E07A2F]/10 rounded-xl flex items-center justify-center">
            <ChefHat className="w-5 h-5 text-[#E07A2F]" />
          </div>
          <div>
            <h1 className="text-2xl font-heading font-bold text-foreground">Commissary</h1>
            <p className="text-sm text-foreground-muted">{totalCount} item{totalCount !== 1 ? "s" : ""}</p>
          </div>
        </div>
        <Link href="/admin/commissary/items/new">
      ...
      {isLoading ? (
        ...
        <Card variant="elevated">
          <CardBody className="py-12 text-center">
            ...
            <h3 className="font-semibold text-foreground mb-2">No commissary items</h3>
            <p className="text-sm text-foreground-muted mb-4">
              {searchQuery ? "No items match your search" : "Get started by adding your first commissary item"}
            </p>
            {!searchQuery && (
              <Link href="/admin/commissary/items/new">
                <Button variant="primary" leftIcon={<Plus className="w-4 h-4" />}>
          ...
          {items.map((item) => {
            ...
            return (
              <Card
                ...
                onClick={() => router.push(`/admin/commissary/items/${item.id}`)}
              >
                <CardBody className="p-4">
