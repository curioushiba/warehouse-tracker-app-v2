...
function getDisplayName(
  ...
  // Fallback to user email if profile is not available or has no name fields
  if (user?.email) {
...
export default function EmployeeHomePage() {
  ...
  const handleMethodSelect = (method: "scan" | "manual") => {
    setModalOpen(false);
    if (method === "scan") {
      router.push(`/employee/scan?type=${selectedAction}`);
    } else {
      // Manual mode - go to scan page with manual mode pre-selected
      router.push(`/employee/scan?type=${selectedAction}&mode=manual`);
    }
  };
  ...
  // Data loading is separate and handled independently
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Hero Action Buttons - Card-based style with elevated icon emphasis */}
      <div className="grid grid-cols-2 gap-4">
        {/* IN Button */}
        <button
        ...
        <ModalBody className="pb-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <button
              ...
              className="group flex flex-col items-center p-5 rounded-card border-2 border-border hover:border-primary hover:bg-primary-50/50 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary/30"
            >
              <div className="w-14 h-14 rounded-full bg-primary-100 flex items-center justify-center mb-3 group-hover:bg-primary-200 transition-colors duration-200">
