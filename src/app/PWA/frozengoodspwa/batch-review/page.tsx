...
export default function FrozenGoodsBatchReviewPage() {
  ...
  const handleBack = () => {
    router.push(`/PWA/frozengoodspwa/scan?type=${apiTransactionType}`);
  };
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      if (newFailedItems.length > 0) {
        ...
        clearBatch();
        router.push("/PWA/frozengoodspwa?batchSuccess=" + totalItems);
      }
    } catch (err) {
      setIsSubmitting(false);
      setShowConfirmModal(false);
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full py-12">
        ...
        <p className="text-sm text-foreground-muted mb-6 text-center">
          Go back to scan mode to add frozen items
        </p>
