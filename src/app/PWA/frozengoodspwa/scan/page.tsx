...
import { getFgItemByCode, getRecentFgItems, searchFgItems } from "@/lib/actions";
import type { Item } from "@/lib/supabase/types";

type ScanMode = "camera" | "manual";
type TransactionType = "check_in" | "check_out";

export default function FrozenGoodsScanPage() {
  ...
  // Fetch recent frozen goods items
  React.useEffect(() => {
    async function fetchRecent() {
      try {
        setIsLoadingRecent(true);
        const result = await getRecentFgItems(4);
  ...
  React.useEffect(() => {
    router.prefetch("/PWA/frozengoodspwa/batch-review");
  }, [router]);
  ...
  const handleCodeScanned = React.useCallback(async (code: string) => {
    ...
    try {
      const result = await getFgItemByCode(code);
      if (result.success && result.data) {
  ...
  const handleDoneScanning = () => {
    router.push("/PWA/frozengoodspwa/batch-review");
  };
          ...
          <>
            ...
            {isLookingUp && (
              <div className="text-center py-2">
                <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#0077b6]/10 text-[#0077b6] rounded-full text-sm">
            ...
            <Card variant="elevated">
              <CardBody>
                <div className="text-center">
                  <div className="w-16 h-16 bg-[#0077b6]/10 rounded-2xl flex items-center justify-center mx-auto mb-3">
                  ...
                  <p className="text-sm text-foreground-muted mt-1">
                    Type to search frozen goods and tap to add
                  </p>
                </div>
              </CardBody>
            </Card>
