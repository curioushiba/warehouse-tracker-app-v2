...
} from "@/lib/actions/frozen-goods-transactions";
...
export default function FrozenGoodsHomePage() {
  ...
  React.useEffect(() => {
    ...
    if (batchSuccess) {
      ...
      if (!isNaN(count) && count > 0) {
        ...
        router.replace("/PWA/frozengoodspwa", { scroll: false });
  ...
  React.useEffect(() => {
    router.prefetch("/PWA/frozengoodspwa/scan");
    router.prefetch("/PWA/frozengoodspwa/history");
  }, [router]);
    ...
    async function fetchData() {
      ...
      try {
        const [recentResult, todayResult] = await Promise.all([
          getFgEmployeeTransactionsWithItems(user.id, { limit: 3 }),
          getFgEmployeeTransactionsWithItems(user.id, { todayOnly: true }),
        ]);
        ...
        console.error('[FrozenGoodsPage] Exception fetching data:', error);
        setRecentTransactions([]);
        setTodayTransactionsCount(0);
      } finally {
        setIsLoadingData(false);
  ...
  const handleMethodSelect = (method: "scan" | "manual") => {
    setModalOpen(false);
    if (method === "scan") {
      router.push(`/PWA/frozengoodspwa/scan?type=${selectedAction}`);
    } else {
      router.push(`/PWA/frozengoodspwa/scan?type=${selectedAction}&mode=manual`);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Welcome Section - Blue frozen goods branding */}
      <div className="bg-gradient-to-br from-[#0077b6] to-[#023e8a] rounded-card p-6 text-white">
        ...
        <div className={`grid ${failedSyncCount > 0 ? 'grid-cols-3' : 'grid-cols-2'} gap-3`}>
          ...
          {failedSyncCount > 0 && (
            <Link href="/PWA/frozengoodspwa/profile" className="bg-error/20 rounded-card p-3 hover:bg-error/30 transition-colors">
        ...
        <ModalBody className="pb-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            <button
              ...
              className="group flex flex-col items-center p-5 rounded-card border-2 border-border hover:border-[#0077b6] hover:bg-[#0077b6]/5 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#0077b6]/30"
            >
              <div className="w-14 h-14 rounded-full bg-[#0077b6]/10 flex items-center justify-center mb-3 group-hover:bg-[#0077b6]/20 transition-colors duration-200">
                <QrCode className="w-7 h-7 text-[#0077b6]" />
              </div>
            ...
            >
              <div className="w-14 h-14 rounded-full bg-[#0077b6]/10 flex items-center justify-center mb-3 group-hover:bg-[#0077b6]/20 transition-colors duration-200">
                <PenLine className="w-7 h-7 text-[#0077b6]" />
              </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Link
            href="/PWA/frozengoodspwa/history"
            className="text-sm text-[#0077b6] font-medium flex items-center gap-1"
          >
            View All
            ...
            {isLoadingData ? (
              ...
              <div className="p-8 text-center">
                ...
                <p className="text-foreground-placeholder text-xs mt-1">
                  Start by checking frozen items in or out
                </p>
              </div>
            ) : (
