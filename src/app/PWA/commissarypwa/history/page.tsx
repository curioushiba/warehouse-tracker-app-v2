...
export default function CommissaryHistoryPage() {
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      const result = await getCmEmployeeTransactionsWithItemsPaginated(user.id, {
        limit: PAGE_SIZE,
        offset: 0,
      });
          ...
          <CardBody className="text-center py-12">
            ...
            <p className="text-sm text-foreground-muted">
              {transactions.length === 0
                ? "You haven't made any commissary transactions yet"
