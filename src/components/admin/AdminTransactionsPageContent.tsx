...
import type { TransactionWithDetails } from "@/lib/actions/_shared/transaction-queries";
import type { AdminTransactionFilters } from "@/lib/actions/_shared/transaction-queries";
...
export interface AdminTransactionsPageContentProps {
  fetchTransactions: (
    filters?: AdminTransactionFilters
  ) => Promise<ActionResult<PaginatedResult<TransactionWithDetails>>>;
  fetchUsers: () => Promise<ActionResult<Profile[]>>;
...
export default function AdminTransactionsPageContent({
  fetchTransactions,
  fetchUsers,
  exportFilePrefix = "transactions",
}: AdminTransactionsPageContentProps) {
  ...
  // Fetch users once on mount (user list is static)
  React.useEffect(() => {
  ...
  const fetchData = React.useCallback(async () => {
    try {
      ...
      const transactionsResult = await fetchTransactions(filters);
  ...
  }, [currentPage, pageSize, debouncedSearch, typeFilter, userFilter, startDate, endDate, fetchTransactions]);
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `${exportFilePrefix}-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
