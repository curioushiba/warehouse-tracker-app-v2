...
import { processQueue, refreshAllCaches } from '@/lib/sync/sync';
...
export function createSyncQueueManager(deps: {
  ...
  async function syncNow(): Promise<void> {
    ...
    try {
      ...
      // Refresh all caches so data is up-to-date
      try {
        await refreshAllCaches(getDb(), getSupabase());
      } catch {
      ...
      const errorMsg =
        result.failed > 0
          ? `${result.failed} production(s) failed to sync`
          : null;
...
/**
 * React hook that manages offline production queue sync.
 *
 * - Automatically syncs when the app returns to the foreground.
 * - Exposes `syncNow` for manual trigger and `refreshCache` for all cache updates.
